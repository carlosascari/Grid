...
export const Column = ({ children, className, border, span, style, auto, size }) => {
  // Column-[size]-span
  const spanClassName = span ? `Column-${ size ? `${ size }-${ span }` : span}` : '';
  return (
    <div className={`Column ${ border ? 'Grid--border' : ''} ${ spanClassName } ${ auto ? 'Column-auto' : ''} ${ className }`} style={style}>
      { children }
    </div>
  )
};
